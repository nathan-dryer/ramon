...
export function ScrapbookGrid({ items, isAdmin }: ScrapbookGridProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center p-8 border-2 border-dashed border-border rounded-lg bg-card/50">
        ...
        <h2 className="font-display text-2xl text-muted-foreground">
          The Scrapbook is Eerily Empty...
        </h2>
        <p className="font-serif text-muted-foreground mt-2">
          Be the first to leave a message or check back soon!
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 sm:gap-6 md:gap-8 p-0 sm:p-2 md:p-4">
      {items.map((item, index) => (
        <div
          key={item.id}
          className="mb-6 md:mb-8 last:mb-0" // Margin bottom for spacing; remove on last for neat end
