    ...
    <header className="w-full">
      <div className="container flex h-16 items-center space-x-4 sm:justify-between sm:space-x-0 px-4 md:px-8">
        <Link href="/login" className="flex items-center space-x-2 group">
          ...
          <span className="font-display text-lg md:text-xl lg:text-2xl font-bold text-foreground group-hover:text-primary transition-colors">
            Ramon's 50th Jubilee!
          </span>
        </Link>
        <AdminIcon isAuthenticated={isAdmin} key={isAdmin ? 'admin-logged-in-header' : 'admin-logged-out-header'} />
      </div>
    </header>
  );
}
